...
#
# The eMMC still bootloops even after baytrail-esp-restore.sh restored vmlinuz.A
...
OUT=/mnt/baytrail-ustate/baytrail-diag-out
TRACE=/mnt/baytrail-ustate/baytrail-diag-out/trace.log
...
mkdir -p "$OUT"
say "=== baytrail-esp-diag.sh PID $$ === USB_DISK=$USB_DISK USTATE=$USTATE (mount took $j tries)"
...
[ -f "$EESP_MNT/baytrail-esp-restore.log" ] && cp -a "$EESP_MNT/baytrail-esp-restore.log" "$OUT/" 2>/dev/null
...
say "diag output saved to USB STATE:/baytrail-diag-out/"
