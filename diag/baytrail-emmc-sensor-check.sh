...
#   - RETRIEVE mode: if a prior diag left /baytrail-sensor-diag.log on eMMC ROOT-A,
#     copy it to USB ROOT-A, restore eMMC grub init=/sbin/init, delete the log.
#   - ARM mode: else copy baytrail-sensor-diag.sh onto eMMC /sbin and point eMMC
#     grub init= at it.
# Flow: boot USB (ARM) -> boot eMMC (diag runs, powers off) -> boot USB (RETRIEVE)
#       -> read /baytrail-emmc-sensor-diag.log on USB ROOT-A here in crosh.
export PATH=/usr/sbin:/usr/bin:/sbin:/bin
CON=/dev/tty1
TRACE=/baytrail-emmc-sensor-check.log
EROOTA_MNT=/mnt/e-root
EESP_MNT=/mnt/e-esp
DIAG=baytrail-sensor-diag.sh
...
say "=== baytrail-emmc-sensor-check.sh PID $$ ==="
...
if [ -f "$EROOTA_MNT/baytrail-sensor-diag.log" ]; then
  say "RETRIEVE: found eMMC diag log — copying to USB ROOT-A and restoring boot"
  cp -f "$EROOTA_MNT/baytrail-sensor-diag.log" /baytrail-emmc-sensor-diag.log 2>>"$TRACE"
  sed -i 's#init=[^ ]*#init=/sbin/init#' "$EESP_MNT/boot/grub/grub.cfg" 2>>"$TRACE"
  rm -f "$EROOTA_MNT/baytrail-sensor-diag.log"
  sync
  MODE="RETRIEVE done — eMMC restored to normal boot; read /baytrail-emmc-sensor-diag.log on USB"
else
