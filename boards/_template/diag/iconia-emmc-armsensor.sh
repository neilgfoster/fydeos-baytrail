...
#   - if eMMC grub init already points at iconia-sensor-boot.sh -> DISARM
#     (restore init=/sbin/init).
#   - else -> ARM: copy iconia-sensor-boot.sh onto eMMC /sbin, set eMMC grub
...
# The wrapper script must be on the USB rootfs at /sbin/iconia-sensor-boot.sh.
export PATH=/usr/sbin:/usr/bin:/sbin:/bin
CON=/dev/tty1
TRACE=/iconia-emmc-armsensor.log
EROOTA_MNT=/mnt/e-root
EESP_MNT=/mnt/e-esp
WRAP=iconia-sensor-boot.sh
...
say "=== iconia-emmc-armsensor.sh PID $$ ==="
