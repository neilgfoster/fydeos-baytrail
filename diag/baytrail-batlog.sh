#!/bin/sh
# baytrail-batlog.sh — lightweight detached battery-drain logger for the before/
...
#
# START (detached):  ssh -i /tmp/ik root@IP 'setsid sh /tmp/baytrail-batlog.sh >/dev/null 2>&1 </dev/null & echo pid $!'
# READ:              (see baytrail-batlog reader one-liner in the campaign notes)
# STOP:              ssh -i /tmp/ik root@IP 'pkill -f baytrail-batlog.sh'
F=/var/log/baytrail-batlog.csv
