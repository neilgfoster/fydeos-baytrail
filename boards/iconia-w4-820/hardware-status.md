...
| Backlight / brightness | 🟡 partial | i915 + PMIC PWM (`PWM_CRC`) | kernel + cmdline | Session 3 after Bay Trail rebuild: backlight device now EXISTS (was absent) at 100% — `PWM_CRC`/`INTEL_SOC_PMIC` found the PMIC PWM. Brightness ADJUSTMENT still not working (wrong backlight picked, or not wired at OOBE). Next: inspect `/sys/class/backlight/*`, try `acpi_backlight=` / write brightness. |
