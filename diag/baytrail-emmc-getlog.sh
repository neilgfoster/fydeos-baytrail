#!/bin/sh
# Iconia W4-820 — pull /baytrail-*.log traces off the eMMC ROOT-A onto the USB
# ROOT-A (PID 1, off USB), so they can be read in crosh (eMMC isn't visible to the
# host). Read-only w.r.t. eMMC boot config. Copies to /baytrail-emmc-<name>.log on
# the USB rootfs (= our own /). Does not change either grub.
export PATH=/usr/sbin:/usr/bin:/sbin:/bin
CON=/dev/tty1
TRACE=/baytrail-emmc-getlog.log
...
say "=== baytrail-emmc-getlog.sh PID $$ ==="
...
cnt=0
for f in "$EROOTA_MNT"/baytrail-*.log; do
  [ -f "$f" ] || continue
  cp -f "$f" "/baytrail-emmc-$(basename "$f")" 2>>"$TRACE" && cnt=$((cnt+1))
done
sync
umount "$EROOTA_MNT" 2>/dev/null
finish "=== GOT $cnt log(s) — read /baytrail-emmc-baytrail-*.log on USB ROOT-A in crosh ===" 12
