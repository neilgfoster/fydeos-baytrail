...
| USB (OTG) | partial | `dwc3` / xhci | kernel | OTG adapter used to attach the installer USB. S16: an **OTG keyboard DOES work at the grub menu / firmware level** (used it to select a kernel entry during recovery) — earlier "OTG keyboard input DEAD" refers to FydeOS *userspace* (no tablet terminal). Keyboard-at-grub lifts the "no keyboard" recovery constraint. |
