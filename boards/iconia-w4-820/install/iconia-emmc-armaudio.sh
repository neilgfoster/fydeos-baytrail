...
# at it (PID 1, off USB). One-shot: iconia-audio-diag.sh self-restores
# init=/sbin/init when it finishes, so the eMMC boots normally again afterward.
# The diag must be present on the USB rootfs at /sbin/iconia-audio-diag.sh.
export PATH=/usr/sbin:/usr/bin:/sbin:/bin
CON=/dev/tty1
TRACE=/iconia-emmc-armaudio.log
EROOTA_MNT=/mnt/e-root
EESP_MNT=/mnt/e-esp
DIAG=iconia-audio-diag.sh
...
say "=== iconia-emmc-armaudio.sh PID $$ ==="
