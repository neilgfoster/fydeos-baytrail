...
# (/iconia-wifi99-diag.log); nothing on the eMMC is modified.
#
# Run via init=/sbin/iconia-wifi99-diag.sh from the USB, read the log on the laptop.
...
TRACE=/iconia-wifi99-diag.log          # on the USB root (bring to laptop)
...
: > "$TRACE" 2>/dev/null || TRACE=/tmp/iconia-wifi99-diag.log
